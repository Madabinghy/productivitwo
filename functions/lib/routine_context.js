...
exports.TIME_CONTEXTS = void 0;
exports.routineTimeContext = routineTimeContext;
exports.TIME_CONTEXTS = {
...
function routineTimeContext(name, overrideKey) {
    if (overrideKey && exports.TIME_CONTEXTS[overrideKey])
        return exports.TIME_CONTEXTS[overrideKey];
    const n = fold(String(name !== null && name !== void 0 ? name : "").trim());
    if (n.length < 3)
        return null;
    for (const a of ARCHETYPES) {
        if (a.keywords.some((k) => n.includes(k)))
            return exports.TIME_CONTEXTS[a.contextKey];
    }
    return null;
}
//# sourceMappingURL=routine_context.js.map