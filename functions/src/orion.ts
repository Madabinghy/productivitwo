...
import {
  ...
  executeDeleteAssistantMessage, executeUpdateActivityGoal, executeSetActivityTargets, executeCreateRoutine,
...
const ORION_TOOLS: PromptCachingBetaTool[] = [
  ...
  { name: "propose_change",          description: "Au lieu de modifier directement les projets, ENREGISTRE une proposition que l'utilisateur validera dans la file « À valider » (accepter/refuser/rediriger). À utiliser pour TOUTE réorganisation structurelle issue de l'inbox : créer un projet, rattacher une idée comme tâche d'un projet, créer un sous-projet, archiver un projet inactif. Tu NE crées PAS le projet/la tâche toi-même — l'app applique la mutation à l'acceptation.", input_schema: { type: "object", required: ["kind", "title", "rationale"], properties: { kind: { type: "string", enum: ["new_project", "attach_idea_as_task", "create_subproject", "archive_project", "add_phase", "attach_action_to_task"], description: "new_project = nouveau projet · attach_idea_as_task = ajouter une tâche à un projet existant · create_subproject = sous-projet d'un projet existant · archive_project = archiver un projet inactif · add_phase = ajouter une phase à un projet existant · attach_action_to_task = ajouter une action à une tâche existante" }, title: { type: "string", description: "Résumé humain court, ex: 'Créer le projet « Refonte site »'" }, rationale: { type: "string", description: "Pourquoi, en 1 phrase, basé sur le contexte/l'idée" }, sourceCaptureId: { type: "string", description: "id de l'idée inbox d'origine (la capture passera en 'proposed')" }, payload: { type: "object", description: "Données pour appliquer la mutation selon kind : new_project={projectTitle, domainId?, description?} · attach_idea_as_task={projectId, taskTitle, description?} · create_subproject={parentProjectId, projectTitle, domainId?} · archive_project={projectId} · add_phase={projectId, phaseLabel, startDate?, endDate?, color?} · attach_action_to_task={projectId, taskId, actionLabel}", properties: { projectId: { type: "string" }, parentProjectId: { type: "string" }, taskId: { type: "string" }, projectTitle: { type: "string" }, taskTitle: { type: "string" }, phaseLabel: { type: "string" }, actionLabel: { type: "string" }, domainId: { type: "string" }, description: { type: "string" }, startDate: { type: "string" }, endDate: { type: "string" }, color: { type: "string" } } } } } },
        ...
        try {
          switch (block.name) {
            ...
            case "set_activity_targets":
              result = await executeSetActivityTargets(uid, args as Parameters<typeof executeSetActivityTargets>[1]);
            ...
            // ── Objectifs stratégiques ─────────────────────────────────────
            case "save_objective":
              result = await executeSaveObjective(uid, args as unknown as Parameters<typeof executeSaveObjective>[1]);
              actionLog.push(`🎯 Objectif : ${(args.title as string) ?? ""}`);
              break;
            default:
