...
import { importGcalDay } from "./gcal";
...
export const proposeDayPlan = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await importGcalDay(uid, target).catch(() => null);
      ...
      const skipBusy = (st: number, dur: number): number => {
        for (const [a, b] of busy) {
          if (st < b && st + dur > a) return skipBusy(b, dur); // chevauche → après
        }
        return st;
      };
      ...
      const linkByTitle = (
        ...
        if (b.activityId) return b;
        const title = normName(String(b.title ?? ""));
        const matches = routineNames.filter((a) => title.includes(normName(a.name)));
        return matches.length === 1
          ? { ...b, activityId: matches[0].id, category: "routine" }
          : b;
      };
...
export const sendMagicLink = onRequest(
  ...
  async (req, res) => {
    ...
    if (!apiKey) { res.status(500).json({ error: "BREVO_API_KEY non configurée" }); return; }
...
export const orionBrief = onRequest(
  ...
  async (req, res) => {
    ...
    const { action, focus, feedback, date, limit } = req.body as {
      action?: "getBrief" | "setFocus" | "setFeedback" | "getFocus" | "history" | "sweepInbox";
      focus?: string;
    ...
    try {
      ...
      if (action === "sweepInbox") {
        const r = await processInboxToProjects(uid, { force: true });
        if (!r) { res.status(502).json({ error: "Routage indisponible — réessaie." }); return; }
...
export { coachApi, coachConsent, coacheeApi, coachAlertsCron } from "./coaching";
